...
import { addService, deleteService, toggleServiceStatus } from "./actions";
import { ImageUpload } from "@/components/ui/image-upload";
import { Edit, Trash2, EyeOff, Plus, Search, Filter, Briefcase } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
            <Briefcase className="w-5 h-5 text-slate-500" /> Daftar Layanan
          </h2>
        ...
        <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 bg-slate-50/50">
          <div className="relative flex-1">
            ...
            <Input className="pl-9 bg-white" placeholder="Cari layanan..." />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-500 text-xs font-bold uppercase border-b border-slate-200">
              <tr>
                <th className="px-6 py-4">GAMBAR/ICON</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {allServices.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-slate-500">Belum ada layanan.</td>
                </tr>
              ) : (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      {item.imageUrl ? (
                        ...
                        <div className="w-16 h-16 bg-slate-100 rounded border border-slate-200 flex items-center justify-center text-slate-500 text-xs flex-col">
                          {item.icon ? <span className="font-bold">{item.icon}</span> : "No Img"}
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <form action={async () => { "use server"; await toggleServiceStatus(item.id, item.isActive); }}>
      ...
      <div id="add-form" className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mt-8">
        <div className="p-4 border-b border-slate-200 bg-slate-50/50">
          <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
            <Plus className="w-5 h-5 text-slate-500" /> Tambah Layanan Baru
          </h2>
        </div>
        <div className="p-6">
          <form action={addService} className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1 space-y-6">
              <div className="space-y-2">
                ...
                <Input id="title" name="title" required className="bg-white border-slate-300" placeholder="Contoh: Pembuatan Website" />
              </div>
              <div className="space-y-2">
              ...
              <div className="bg-[#f0f9ff] border border-[#bae6fd] rounded-lg p-5 mt-6">
                <h3 className="text-[#0369a1] font-bold text-sm mb-4 flex items-center gap-2">
                  <span className="w-4 h-4 bg-[#bae6fd] text-[#0369a1] flex items-center justify-center rounded-sm text-[10px]">S</span>
                  Spesifikasi Layanan
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="icon" className="text-slate-700 font-bold text-xs uppercase tracking-wider">Icon (Nama Lucide)</Label>
                    <Input id="icon" name="icon" className="bg-white border-slate-300" placeholder="Contoh: Code, Laptop" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="basePrice" className="text-slate-700 font-bold text-xs uppercase tracking-wider">Harga Dasar (Rp)</Label>
                    <Input id="basePrice" name="basePrice" type="number" className="bg-white border-slate-300" placeholder="Contoh: 5000000" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="w-full lg:w-[320px] space-y-6">
              <div>
                <Label className="text-slate-700 font-bold text-xs uppercase tracking-wider block mb-2">Gambar Representasi (Opsional)</Label>
                <p className="text-xs text-slate-500 mb-4">Jika ingin memakai gambar alih-alih icon.</p>
              ...
              <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-semibold text-slate-700">Aktifkan Layanan</span>
                </label>
